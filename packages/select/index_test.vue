...
const vClickOutside ={
    beforeMount (el) {
        let handler = (e) =>{
            ...
            if(!props.disabled){
                if(!props.multiple){
                    // console.log(e.targe.className);
                    // e.target.className.indexOf("mzl-select-option-li") == '-1' 加入淡入淡出变化后有问题 当你没找到该样式且点击时 出现下拉框 当你已经点击过后 则该样式已经出现 下拉框消失
                ...
                //复选框
                    ...
                    //这里不需要跟单选框一样 因为不是点击一个选项后下拉框就消失了
...
const selChange  = (item,index) =>{
    ...
    if(!props.multiple){
        ...
        if(!item.disabled){
            ...
            emit("change",{label:labels , value: selVal.value ,index:indexs})
